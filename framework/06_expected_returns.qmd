---
title: "6. Expected returns: what the portfolio should earn from here"
---
...
#| echo: false
#| warning: false
#| fig-cap: "Shiller CAPE ratio at start of period versus subsequent 10-year annualized real total return, S&P 500, monthly observations 1881–2015. The negative OLS slope is statistically significant and persistent across the full sample. High starting valuations have historically predicted lower subsequent long-horizon returns. Source: Shiller (2023)."
...
## Corporate margins and the hidden stability assumption

Most significant errors in expected return work do not come from arithmetic. They come from a single embedded assumption that is never stated explicitly: that the structural conditions underlying the forecast will remain roughly stable.

The corporate profit margin chart illustrates one version of this problem. U.S. corporate profits after tax as a share of GDP expanded substantially from the early 2000s through 2021, reaching levels that were historically elevated by any long-run standard. Many equity return forecasts produced during that period treated the elevated margin as a durable feature of the competitive landscape — the result of technology, globalization, and low labor bargaining power — rather than as a cyclical high that would eventually mean-revert. When margins compressed in 2022, the revenue growth that analysts had been extrapolating forward did not translate into earnings growth at the rate the models assumed, and equity valuations proved more sensitive to the combined effect of margin compression and rising discount rates than most forecasts had embedded.

```{python}
#| echo: false
#| warning: false
#| fig-cap: "U.S. corporate profits after tax as a share of GDP, 1947–2023. The post-2008 expansion to historically elevated levels, the 2022 compression, and prior mean-reversion episodes are all visible. An expected return forecast that treats peak margins as structural is importing a stability assumption that history does not support. Source: FRED (CP, GDP)."
show("fig_corporate_margins.png")
```

The general principle is that every expected return forecast contains embedded assumptions about structural continuity — about margins, about the policy regime, about diversification relationships, about the cost and availability of financing. These assumptions are invisible when they hold and consequential when they fail. The improvement that matters most in expected return work is not more elaborate modeling but more honest accounting of which assumptions are carrying the forecast and what their failure would imply for the stated return. A forecast that can answer that question explicitly — here is what must be true for this return to be realized, and here is what happens to the return if it is not — is more useful than a more precise forecast that cannot.

That standard applies directly to the portfolio whose construction section 5 described and whose active additions section 7 evaluates. The question at every stage is not only what the expected return is but what it depends on — and whether that dependency is consciously accepted or merely inherited from a projection that has not been examined carefully enough.